...
/***/ function(module, exports, __webpack_require__) {
	...
	function fromRandomAccessReader(reader, totalSize, options, callback) {
	  ...
	  readAndAssertNoEof(reader, buffer, 0, bufferSize, bufferReadStart, function(err) {
	    ...
	    for (var i = bufferSize - eocdrWithoutCommentSize; i >= 0; i -= 1) {
	      ...
	      var zip64EocdlBuffer = Buffer.alloc(20);
	      ...
	      readAndAssertNoEof(reader, zip64EocdlBuffer, 0, zip64EocdlBuffer.length, zip64EocdlOffset, function(err) {
	        ...
	        var zip64EocdrBuffer = Buffer.alloc(56);
	  ...
	  readAndAssertNoEof(self.reader, buffer, 0, buffer.length, self.readEntryCursor, function(err) {
	    ...
	    readAndAssertNoEof(self.reader, buffer, 0, buffer.length, self.readEntryCursor, function(err) {
	      ...
	      while (i < extraFieldBuffer.length - 3) {
	        ...
	        var dataBuffer = Buffer.alloc(dataSize);
	...
	function readAndAssertNoEof(reader, buffer, offset, length, position, callback) {
	  if (length === 0) {
	    ...
	    return setImmediate(function() { callback(null, Buffer.alloc(0)); });
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	function ensureBuffer(input) {
	  ...
	  if (typeof input === "number") {
	    return hasNewBufferAPI ? Buffer.alloc(input) : Buffer.from(input);
	  }
	  else if (typeof input === "string") {
	    return hasNewBufferAPI ? Buffer.from(input) : Buffer.from(input);
	  }
	  else {
